...
$log_activity = "mengakses data user level 2";
...
$counter = 1;
$sql = "select * from user, user_detail where user.id=user_detail.id and user.level=4";
...
while($commandCounter < $stringCounter){
    ...
    if($commander == "-"){
        ...
        if($commanders == "-"){
            $query = mysql("u6799722_rumahsakit_dev", "select nama from u6799722_rumahsakit_dev.user_detail where user_detail.id='$commander'");
            $row = mysql_fetch_array($query);
            $dataPerangkat = $row['nama'];
